    ...
    return (
      <main className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">Take a look at our menu and choose your favorite dishes!</p>
          <Link href="/foods" className="bg-rose-500 hover:bg-rose-600 text-white font-medium px-6 py-3 rounded-xl transition-colors">
            Explore Menu Now
          </Link>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-gray-50 py-12 px-6">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden p-6">
            <div className="divide-y divide-gray-100">
              {cart.map((item) => (
                <div key={item.dishId} className="py-4 flex justify-between items-center gap-4">
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">{item.name}</h3>
                    <p className="text-sm text-gray-500">Restaurant: {item.restaurantName || 'N/A'}</p>
                    {item.note && <p className="text-xs text-amber-600 mt-0.5">Note: {item.note}</p>}
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                      ...
                      <span className="px-3 py-1 text-center font-medium text-sm w-10">{item.quantity}</span>
                      <button 
                        onClick={() => updateQuantity(item.dishId, item.quantity + 1)}
                        className="px-2.5 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold text-sm"
                      >
                    ...
                    <div className="text-right min-w-[90px]">
                      ...
                      <button onClick={() => removeFromCart(item.dishId)} className="text-red-500 hover:text-red-700 text-xs font-medium">
                        Remove
                      </button>
                    </div>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 text-right">
               <button onClick={clearCart} className="text-gray-400 hover:text-gray-600 text-sm">Clear All</button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-fit space-y-6">
          <h2 className="text-xl font-bold text-gray-900 border-b pb-4">Delivery Information</h2>
          ...
          <div className="border-t border-gray-100 pt-4 flex justify-between items-center">
            ...
            <span className="text-2xl font-extrabold text-rose-600">{totalAmount.toLocaleString('en-US')} VND</span>
          </div>
